<x-app-layout>
    ...
    <div class="p-4 sm:p-6 lg:p-8">
        ...
        <x-breadcrumb :items="[
            ...
            ['label' => 'Tambah Proses QC']
        ]" />

        <div class="max-w-3xl mx-auto">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-lg rounded-lg">
                <div class="p-6">
                    <form action="{{ route('qc-summary.store') }}" method="POST" x-data="qcForm()">
                        @csrf
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select name="process" id="process"
                                    ...
                                    <option value="hanging" {{ old('process') == 'hanging' ? 'selected' : '' }}>Gantungan</option>
                                    <option value="thread_trimming" {{ old('process') == 'thread_trimming' ? 'selected' : '' }}>Buang Benang</option>
                                    <option value="buttoning" {{ old('process') == 'buttoning' ? 'selected' : '' }}>Kancing</option>
                                    <option value="plating" {{ old('process') == 'plating' ? 'selected' : '' }}>Plat</option>
                                    <option value="steaming" {{ old('process') == 'steaming' ? 'selected' : '' }}>Steam</option>
                                </select>
                                @error('process')
                                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
